...
async fn generate_with_tippecanoe(
    ...
    let output = cmd
        .output()
        .await
        .context(format!("Failed to run '{}'. Is it installed and in PATH?", bin))?;
...
async fn generate_with_gdal(source: &SourceConfig, reader: &PostgisReader) -> Result<()> {
    ...
    for (i, (layer_name, geojson_path)) in geojson_files.iter().enumerate() {
        ...
        // Zoom range
        cmd.arg("-dsco")
            .arg(format!("MINZOOM={}", source.min_zoom));
        cmd.arg("-dsco")
            .arg(format!("MAXZOOM={}", source.max_zoom));
        ...
        let output = cmd.output().await.context(
            "Failed to run 'ogr2ogr'. Is GDAL installed and in PATH?",
        )?;
...
async fn generate_with_native(source: &SourceConfig, reader: &PostgisReader) -> Result<()> {
    ...
    store.write_default_metadata(&source.name, &format!("Generated by tilefeed for {}", source.name))?;

    // Build layer configs map for encode_tile_with_config
    let layer_configs: HashMap<String, &crate::config::LayerConfig> = source
        .layers
        .iter()
    ...
    for z in source.min_zoom..=source.max_zoom {
        ...
        for x in 0..=max_tile {
            for y in 0..=max_tile {
                ...
                for layer in &source.layers {
                    let features =
                        reader.get_features_in_bounds(layer, &bounds).await?;
